...
async function createPayment(type, amount, config, options = {}) {
  ...
  if (type === "payvalls") {
    ...
    // Generate QR dari payment link
    const qrDir = path.join(__dirname, "temp_qr");
    if (!fs.existsSync(qrDir)) fs.mkdirSync(qrDir, { recursive: true });
    const filePath = path.join(qrDir, `${tx.tx_id}.png`);
    await QRCode.toFile(filePath, tx.payment_link, { type: "png", width: 500, margin: 2 });

    setTimeout(() => { try { fs.unlinkSync(filePath); } catch {} }, 120000);
