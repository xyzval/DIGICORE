...
async function createPayment(type, amount, config, options = {}) {
  ...
  if (type === "violet") {
    ...
    if (!apiKey || !secretKey) throw new Error("Config Violet belum lengkap: apiKey/secretKey kosong");
    ...
    try {
      ...
      throw new Error(`Gagal koneksi ke Violet: ${err.message}`);
    }
    ...
    if (!response?.status || !trx || String(trx.status || "").toLowerCase() === "invalid") {
      ...
      throw new Error(`Gagal membuat transaksi Violet: ${msg}`);
    }
    ...
    if (!qris && !trx.checkout_url) {
      ...
      throw new Error("Transaksi Violet berhasil dibuat, tapi QRIS/checkout_url tidak ditemukan");
    }
...
async function cekPaid(type, data, config, extra = {}) {
  ...
  if (type === "pakasir") {
    ...
    if (!orderId) throw new Error("Order ID tidak ditemukan untuk cek Pakasir");

    const url = `https://app.pakasir.com/api/transactioncheck/qris`;
    const body = {
      project: slug,
      order_id: orderId,
      api_key: apiKey
    ...
    try {
      ...
      console.error("[PAKASIR CHECK]", err.message);
      return false;
    }
  }
