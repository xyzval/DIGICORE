...
async function cekPaid(type, data, config, extra = {}) {
  ...
  if (type === "pakasir") {
    ...
    try {
      const res = await axios.post("https://app.pakasir.com/api/transactionstatus/qris", {
        project: slug,
        order_id: orderId,
        api_key: apiKey
      }, {
      ...
      const result = res.data;
      const status = (result?.payment?.status || result?.status || "").toLowerCase();
      if (status === "paid" || status === "settlement" || status === "success" || status === "completed") {
        return true;
      }
    } catch (err) {
