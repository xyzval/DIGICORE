...
async function cekPaid(type, data, config, extra = {}) {
  ...
  if (type === "pakasir") {
    ...
    if (!orderId) {
      console.log("[PAKASIR CHECK] orderId kosong!", { data: JSON.stringify(data).slice(0, 500) });
      return false;
    }

    // Coba beberapa endpoint yang mungkin digunakan Pakasir
    const endpoints = [
      { url: "https://app.pakasir.com/api/transactionstatus/qris", method: "POST" },
      { url: "https://app.pakasir.com/api/transaction/status", method: "POST" },
      { url: "https://app.pakasir.com/api/cek-status", method: "POST" },
    ];
    ...
    for (const ep of endpoints) {
      try {
        ...
        const result = res.data;
        console.log(`[PAKASIR CHECK] ${ep.url} →`, JSON.stringify(result).slice(0, 500));
        ...
        if (status === "paid" || status === "settlement" || status === "success" || status === "completed" || status === "1") {
          console.log("[PAKASIR CHECK] PAID! orderId:", orderId);
          return true;
        }

        // Kalau dapat response valid (bukan 404/500), tidak perlu coba endpoint lain
        if (res.status === 200 && result && !result.error) break;

      } catch (err) {
        console.log(`[PAKASIR CHECK] Error ${ep.url}:`, err.message);
      }
    }
